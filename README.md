...
**Perbandingan Implementasi Fungsi `new` dan `build`:**

Dalam mendirikan instansiasi perlengkapan `ThreadPool`, sebelumnya saya mengeksekusi inisiasi primitif menggunakan metode asertif `new` yang akan melempar *panic* sistem internal (`assert!(size > 0)`) apabila kapasitas dialokasikan bervolume nol. Strategi ini sangatlah barbar di industri aplikasi nyata karena dianggap eksklusif sebagai *unrecoverable error* yang merobohkan keseluruhan program seketika begitu *crash/bug* spesifik terjadi. 

Sebaliknya pada modifikasi perbaikan jenjang Bonus ini, fungsi `new` diganti perannya penuh oleh konstruktor `build` berkonsepsi idiomatik standar yang me-*return* paket nilai berjenis `Result<ThreadPool, PoolCreationError>`.
Desain *architecture* perangkat lunak tingkat ini terbilang **jauh lebih elegan dan informatif** karena galat logika yang ditimbulkan akan ditangkap secara proaktif di jalur perlintasan sebagai *recoverable error*. Polisi verifikasi nilai *return* bertipe `Result Enum` memungkinkan siapapun mengendalikan dan memfasilitasi pelarian penjinakan aman di area *call-site* (misalnya dengan implementasi `.unwrap_or_else()`). Teknik inilah yang melatih sistem dalam print *error traceback* bermakna daripada sekadar  "*panic! at...*" bawaan tanpa penjelasan. Program bisa bermanuver menindaklanjuti kegagalan instansiasi secara sadar tanpa menyeburkan web *server* pada lubang mematikan alias terjun payung menukik tak bisa dikembalikan.
